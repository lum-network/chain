...
func (suite *KeeperTestSuite) TestCallbacks_TransfersFromNative() {
	...
	callbackData := millionstypes.TransferFromNativeCallback{
		PoolId:       pool.GetPoolId(),
		WithdrawalId: 1,
		DrawId:       1,
		Type:         millionstypes.TransferType_Claim,
	}

	// Serialize our callback
	marshalledCallbackArgs, err := suite.App.MillionsKeeper.MarshalTransferFromNativeCallbackArgs(suite.Ctx, callbackData)
	...
	unmarshalledCallbackData, err := suite.App.MillionsKeeper.UnmarshalTransferFromNativeCallbackArgs(suite.Ctx, data.CallbackArgs)
	suite.Require().NoError(err)
	...
	suite.Require().Equal(callbackData.Type, unmarshalledCallbackData.Type)
}
