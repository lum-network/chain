...
func (suite *KeeperTestSuite) TestCallbacks_Redelegates() {
	...
	marshalledCallbackArgs, err := suite.app.MillionsKeeper.MarshalRedelegateCallbackArgs(suite.ctx, callbackData)
	...
	callback := icacallbackstypes.CallbackData{
		...
		ChannelId:    channelID,
		Sequence:     sequence,
		CallbackId:   millionskeeper.ICACallbackID_Redelegate,
	...
	unmarshalledCallbackData, err := suite.app.MillionsKeeper.UnmarshalRedelegateCallbackArgs(suite.ctx, data.CallbackArgs)
	suite.Require().NoError(err)
