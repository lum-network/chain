...
// BlockRedelegateUpdates runs all expired redelegate updates (after completion time)
// Called in each EndBlock
func (k Keeper) BlockRedelegateUpdates(ctx sdk.Context) (successCount, errorCount int) {
	logger := k.Logger(ctx).With("ctx", "blocker_redelegations")

	// Expired Redelegation
	expiredRedelegatedVals := k.DequeueEPRQueue(ctx, ctx.BlockTime())
	...
	if successCount+errorCount > 0 {
		logger.Info(
			"redelegation status update started",
			"nbr_success", successCount,
			"nbr_error", errorCount,
		)
	}
